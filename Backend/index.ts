...
databaseService.initialize().then(() =>{
    ...
    app.listen(9999);
    logger.info("[index][databaseService.initialize]","Server listening on port 9999");
}).catch((error ) =>{
    console.log(error);
    logger.error("[index][databaseService.initialize]","Error while starting express server");
})

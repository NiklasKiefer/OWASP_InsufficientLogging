...
const doTermFunc = (termQuery) => {
    return (...args) => {
        const last = args.pop();
        const tb = (0, query_1.termBuilder)(query_config_1.funcall, toQuery);
...
function toQuery(term) {
    const query = (0, query_1.termBuilder)(query_config_1.bracket, toQuery, term);
    query.term = term;
    query[query_1.querySymbol] = true;
    query.toString = () => (0, term_backtrace_1.backtraceTerm)(term)[0];
    ...
    for (let i = 0; i < query_config_1.termConfig.length; i += 1) {
        ...
        query[config[1]] = (0, query_1.termBuilder)(config, toQuery, term);
    }
    return query;
}

...
class RethinkDBSocket extends events_1.EventEmitter {
    ...
    async connect() {
        ...
        try {
            const socket = await new Promise((resolve, reject) => {
                const s = tls
                    ? (0, tls_1.connect)(options)
                    : (0, net_1.connect)(options);
    ...
    async performHandshake() {
        ...
        const { randomString, authBuffer } = (0, handshake_utils_1.buildAuthBuffer)(this.user);
        generateRunningQuery();
        generateRunningQuery();
        this.socket.write(authBuffer);
        (0, handshake_utils_1.validateVersion)(await this.readNext(0));
        const { authentication } = await this.readNext(1);
        const { serverSignature, proof } = await (0, handshake_utils_1.computeSaltedPassword)(authentication, randomString, this.user, this.password);
        generateRunningQuery();
        this.socket.write(proof);
        const { authentication: returnedSignature } = await this.readNext(2);
        (0, handshake_utils_1.compareDigest)(returnedSignature, serverSignature);
        this.mode = 'response';
    }
    handleHandshakeData() {
        let index = -1;
